...
export default function Package() {
    ...
    const handlePurchase = async () => {
        try {
            ...
            if (!token) {
                Alert.alert("❌ Error", "Bearer token missing. Please re-register.");
                return;
            }
            ...
            if (res.data.success === "success") {
                Alert.alert("🎉 Purchase Successful", res.data.message);
            } else {
                Alert.alert("⚠️ Failed", res.data.message || "Something went wrong.");
            }
        } catch (err) {
